...
// ─── Mobile Nav CSS addition ──────────────────────────────────────────────────
const mobileCss = `
  .hamburger { display: none; flex-direction: column; gap: 5px; cursor: pointer; padding: 4px; background: none; border: none; }
  .hamburger span { width: 22px; height: 2px; background: var(--text); border-radius: 2px; transition: all 0.2s; display: block; }
  .mobile-menu { display: none; position: fixed; top: 61px; left: 0; right: 0; background: var(--bg2); border-bottom: 1px solid var(--border); padding: 16px; z-index: 99; flex-direction: column; gap: 8px; }
...
function TermsPage({ setPage }) {
  return (
    <div style={{ minHeight:"100vh", background:"var(--bg)", padding:"100px 24px 60px", maxWidth:800, margin:"0 auto" }}>
      <button onClick={() => setPage("home")} style={{ background:"none", border:"none", color:"var(--text2)", cursor:"pointer", fontSize:14, marginBottom:32, fontFamily:"DM Sans,sans-serif", display:"flex", alignItems:"center", gap:8 }}>← Back to Home</button>
      <div style={{ fontFamily:"Syne,sans-serif", fontSize:32, fontWeight:800, marginBottom:8 }}>Terms of Service</div>
      <div style={{ color:"var(--text2)", fontSize:14, marginBottom:40 }}>Last updated: January 1, 2026</div>
      {[
        { title:"1. Acceptance of Terms", body:"By accessing and using ReviewShield (reviewshield.in), you agree to be bound by these Terms of Service. If you do not agree to these terms, please do not use our service." },
        { title:"2. Description of Service", body:"ReviewShield provides a reputation management tool that allows businesses to collect customer feedback and encourage satisfied customers to share their experiences on Google. Our service routes customer feedback through a private system before any public review is posted." },
        { title:"3. User Responsibilities", body:"You agree to use ReviewShield only for lawful purposes. You are responsible for maintaining the confidentiality of your account credentials. You agree not to misrepresent your business or use the service to generate fake reviews." },
        { title:"4. Payment Terms", body:"Subscription fees are billed monthly. All payments are processed securely through Razorpay. You may cancel your subscription at any time. No refunds are provided for partial months. Prices are in Indian Rupees (INR) and include applicable taxes." },
        { title:"5. Data & Privacy", body:"We collect business registration information and customer feedback data. We do not sell your data to third parties. Customer feedback captured through our system is stored securely and only accessible to the registered business owner. See our Privacy Policy for full details." },
        { title:"6. Limitation of Liability", body:"ReviewShield is not responsible for the content of customer reviews posted on third-party platforms such as Google. We do not guarantee any specific improvement in ratings or review count. Our liability is limited to the amount paid for the service in the preceding month." },
        { title:"7. Termination", body:"We reserve the right to terminate accounts that violate these terms. You may terminate your account at any time by contacting support. Upon termination, access to the dashboard will be revoked at the end of the billing cycle." },
        { title:"8. Contact", body:"For any questions about these Terms, contact us at: legal@reviewshield.in or call 9911591033." },
      ].map((s, i) => (
        <div key={i} style={{ marginBottom:28 }}>
          <div style={{ fontFamily:"Syne,sans-serif", fontSize:18, fontWeight:700, marginBottom:10, color:"var(--gold)" }}>{s.title}</div>
          <div style={{ fontSize:15, color:"var(--text2)", lineHeight:1.8 }}>{s.body}</div>
        </div>
      ))}
    </div>
  );
...
function PrivacyPage({ setPage }) {
  return (
    <div style={{ minHeight:"100vh", background:"var(--bg)", padding:"100px 24px 60px", maxWidth:800, margin:"0 auto" }}>
      <button onClick={() => setPage("home")} style={{ background:"none", border:"none", color:"var(--text2)", cursor:"pointer", fontSize:14, marginBottom:32, fontFamily:"DM Sans,sans-serif", display:"flex", alignItems:"center", gap:8 }}>← Back to Home</button>
      <div style={{ fontFamily:"Syne,sans-serif", fontSize:32, fontWeight:800, marginBottom:8 }}>Privacy Policy</div>
      <div style={{ color:"var(--text2)", fontSize:14, marginBottom:40 }}>Last updated: January 1, 2026</div>
      {[
        { title:"1. Information We Collect", body:"We collect: (a) Business information provided during registration including name, email, phone number, and Google Business profile URL. (b) Customer feedback submitted through our review collection system including star ratings, written feedback, and optional name. (c) Usage data such as number of QR code scans and review link clicks." },
        { title:"2. How We Use Your Information", body:"We use collected information to: provide and improve our service, send account and billing notifications, generate analytics for your dashboard, and communicate product updates. We do not use your data for advertising or sell it to third parties." },
        { title:"3. Data Storage", body:"Business and feedback data is stored securely in Google Sheets infrastructure. Payment data is handled exclusively by Razorpay and is never stored on our servers. We follow industry-standard security practices to protect your data." },
        { title:"4. Customer Data", body:"Feedback submitted by your customers through your ReviewShield link is stored privately and is only accessible to you (the business owner) and ReviewShield administrators. This data is never shared with third parties or used for any purpose other than displaying it in your dashboard." },
        { title:"5. Cookies", body:"ReviewShield uses minimal cookies for session management (keeping you logged in). We do not use tracking cookies or third-party advertising cookies." },
        { title:"6. Your Rights", body:"You have the right to access, correct, or delete your data at any time. To request data deletion, email us at privacy@reviewshield.in. We will process all requests within 7 business days." },
        { title:"7. Contact", body:"For privacy concerns: privacy@reviewshield.in | The Design Shore, New Delhi, India | Phone: 9911591033" },
      ].map((s, i) => (
        <div key={i} style={{ marginBottom:28 }}>
          <div style={{ fontFamily:"Syne,sans-serif", fontSize:18, fontWeight:700, marginBottom:10, color:"var(--gold)" }}>{s.title}</div>
          <div style={{ fontSize:15, color:"var(--text2)", lineHeight:1.8 }}>{s.body}</div>
...
function Nav({ page, setPage, currentBusiness }) {
  ...
  const links = currentBusiness
    ? [{ label:"Dashboard", action:() => setPage("dashboard") }, { label:"Preview Page", action:() => setPage("review-preview"), primary:true }]
    : [{ label:"Login", action:() => setPage("login") }, { label:"Start Free Trial", action:() => setPage("register"), primary:true }];
  return (
    <>
      ...
      <nav className="nav">
        ...
        <div className="nav-links">
          {links.map((l, i) => (
            <button key={i} className={`nav-btn ${l.primary ? "primary" : "ghost"}`} onClick={() => { l.action(); setMenuOpen(false); }}>{l.label}</button>
          ))}
        </div>
      ...
      <div className={`mobile-menu ${menuOpen ? "open" : ""}`}>
        {links.map((l, i) => (
          <button key={i} className={`nav-btn ${l.primary ? "primary" : "ghost"}`} style={{ width:"100%", textAlign:"left" }} onClick={() => { l.action(); setMenuOpen(false); }}>{l.label}</button>
        ))}
      </div>
    </>
  );
...
function HomePage({ setPage }) {
  return (
    <div>
      ...
      {/* Footer */}
      <footer style={{ textAlign: "center", padding: "32px 24px", color: "var(--text2)", fontSize: 13, borderTop: "1px solid var(--border)" }}>
        <div style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, color: "var(--gold)", marginBottom: 12, fontSize: 18 }}>ReviewShield</div>
        <div style={{ display:"flex", justifyContent:"center", gap: 20, marginBottom: 12, flexWrap:"wrap" }}>
          {[["Terms of Service","terms"],["Privacy Policy","privacy"],["Contact Us","mailto:tarun@thedesignshore.com"]].map(([label, href], i) => (
            <a key={i} onClick={() => href.startsWith("mailto") ? window.open(href) : setPage(href)}
              style={{ color:"var(--text2)", cursor:"pointer", textDecoration:"none" }}
              onMouseEnter={e => e.target.style.color="var(--gold)"} onMouseLeave={e => e.target.style.color="var(--text2)"}>
              {label}
            </a>
          ))}
        </div>
        <div>© 2026 ReviewShield · Made in India 🇮🇳 · Powered by The Design Shore</div>
      </footer>
    </div>
  );
}

function RegisterPage({ setPage, setCurrentBusiness, setBusinesses, dbConnected }) {
  const [form, setForm] = useState({ name: "", email: "", phone: "", business: "", googleUrl: "", plan: "pro", password: "" });
  ...
  const handleCreateAccount = () => {
    ...
    setTimeout(() => {
      const biz = {
        ...
        email: form.email,
        phone: form.phone,
        password: form.password || "reviewshield123", // default if blank
      ...
      // ✅ Save permanently to Google Sheet
      if (dbConnected) SheetDB.saveBusiness(biz);
      setLoading(false);
      setStep("payment");
    }, 800);
  };
      ...
      {step === "details" && (
        <div className="auth-card fade-in">
          ...
          <div className="form-group">
            ...
            <input className="form-input" placeholder="Min 8 characters" type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} />
          </div>
          <div className="form-group">
...
function LoginPage({ setPage, setCurrentBusiness, businesses }) {
  const [form, setForm] = useState({ email: "", password: "" });
  const [error, setError] = useState(false);

  const handleLogin = () => {
    const found = businesses.find(b => b.email?.toLowerCase() === form.email.toLowerCase());
    if (found) {
      // check password if stored, otherwise just email match
      if (found.password && form.password && found.password !== form.password) {
        setError("wrong_password"); return;
      }
      STORE.currentBusiness = found;
      setCurrentBusiness(found);
      setPage("dashboard");
    } else if (form.email === "owner@spicegarden.com") {
  ...
  return (
    <div className="auth-page">
      <div className="auth-card fade-in">
        ...
        <div className="form-group">
          <label className="form-label">EMAIL ADDRESS</label>
          <input className="form-input" placeholder="you@business.com" type="email" value={form.email}
            onChange={e => { setForm({ ...form, email: e.target.value }); setError(false); }}
            onKeyDown={e => e.key === "Enter" && handleLogin()} />
        </div>
        <div className="form-group">
          <label className="form-label">PASSWORD</label>
          <input className="form-input" placeholder="••••••••" type="password" value={form.password}
            onChange={e => setForm({ ...form, password: e.target.value })}
            onKeyDown={e => e.key === "Enter" && handleLogin()} />
        </div>
        {error === "not_found" && <div style={{ color:"var(--red)", fontSize:13, marginBottom:8 }}>❌ No account found with that email. Please register first.</div>}
        {error === "wrong_password" && <div style={{ color:"var(--red)", fontSize:13, marginBottom:8 }}>❌ Wrong password. Please try again.</div>}
        <div style={{ fontSize: 13, color: "var(--text2)", marginBottom: 8 }}>Demo: <strong style={{color:"var(--gold)"}}>owner@spicegarden.com</strong> + any password</div>
...
function Dashboard({ currentBusiness, setPage, setCurrentBusiness, dbConnected }) {
  ...
  // ── Pro access = paid Pro/Agency plan OR whitelisted email ──────────────────
  const hasPro = biz.plan === "pro" || biz.plan === "agency" || isWhitelisted(biz.email);
        ...
        <div className="sidebar-bottom">
          <div style={{ padding: "0 16px 12px", fontSize: 13, color: "var(--text2)" }}>
            ...
            <div style={{ marginTop: 6, display: "flex", gap: 6, flexWrap: "wrap" }}>
              <div style={{ display: "inline-flex", background: "rgba(245,200,66,0.1)", color: "var(--gold)", fontSize: 11, padding: "2px 10px", borderRadius: 100, fontWeight: 700 }}>{biz.plan.toUpperCase()}</div>
              {isWhitelisted(biz.email) && (
                <div style={{ display: "inline-flex", background: "rgba(168,85,247,0.15)", color: "#a78bfa", fontSize: 11, padding: "2px 10px", borderRadius: 100, fontWeight: 700 }}>⭐ VIP</div>
              )}
            </div>
          </div>
          ...
          <div className="fade-in">
            ...
            <div className="panel">
              ...
              <div className="qr-block">
                ...
                <div style={{ marginTop: 10, textAlign: "center", display:"flex", gap:8, flexWrap:"wrap", justifyContent:"center" }}>
                  <button className="btn-copy" onClick={() => { const a = document.createElement('a'); a.href = qrUrl; a.download = 'reviewshield-qr.png'; a.click(); }}>⬇ Download QR</button>
                  <button className="btn-copy" onClick={() => window.open(`https://wa.me/?text=Rate us on Google! ${reviewLink}`, "_blank")}>📲 Share on WhatsApp</button>
                </div>
                </div>
        ...
        {activeTab === "settings" && (
          <div className="fade-in">
            ...
            <div className="panel">
              ...
              {[
                { key: "emailAlerts", label: "Email Alerts", desc: "Get an email when feedback is captured", pro: false },
                { key: "smsAlerts", label: "SMS Alerts", desc: "Get an SMS for every new private feedback", pro: true },
                { key: "autoreply", label: "Auto-Reply to Feedback", desc: "Automatically thank customers for feedback", pro: true },
              ].map(s => (
                <div className="settings-row" key={s.key}>
                  <div>
                    <div className="settings-label">
                      {s.label}
                      {s.pro && !hasPro && <span style={{ marginLeft: 8, fontSize: 11, background: "rgba(245,200,66,0.1)", color: "var(--gold)", padding: "2px 8px", borderRadius: 100, fontWeight: 700 }}>PRO</span>}
                      {s.pro && hasPro && <span style={{ marginLeft: 8, fontSize: 11, background: "rgba(168,85,247,0.15)", color: "#a78bfa", padding: "2px 8px", borderRadius: 100, fontWeight: 700 }}>✓ UNLOCKED</span>}
                    </div>
                  ...
                  {(!s.pro || hasPro) ? (
                    ...
                    <button onClick={() => setActiveTab("billing")} style={{ fontSize: 12, padding: "5px 12px", borderRadius: 8, border: "1px solid var(--gold)", background: "rgba(245,200,66,0.08)", color: "var(--gold)", cursor: "pointer", fontFamily: "DM Sans,sans-serif" }}>Upgrade</button>
                  )}
                </div>
              ))}
            </div>
...
function ReviewPage({ businessId, businesses, setPage, addFeedback, addPositive, isDemo }) {
  const biz = isDemo
    ? { id: "demo", name: "Spice Garden Restaurant", googleReviewUrl: "https://search.google.com/local/writereview?placeid=ChIJN1t_tDeuEmsRUsoyG83frY4" }
  ...
  const handleRate = () => {
    ...
    if (rating >= THRESHOLD) {
      ...
      // ✅ Increment positivesSent counter
      if (!isDemo && addPositive) addPositive(biz.id);
      setTimeout(() => { window.open(biz.googleReviewUrl, "_blank"); }, 1500);
    } else {
      setStep("feedback");
    }
  };
  ...
  return (
    <div className="review-page">
      <div className="review-card fade-in">
        <div className="review-logo">Review<span style={{ color: "var(--text)" }}>Shield</span></div>
...
// ─── Admin Whitelist — add emails here to give free Pro access ────────────────
const PRO_WHITELIST = [
  "tarun@thedesignshore.com",   // yourself
  // "friend@gmail.com",        // add any email here to give free Pro
  // "family@gmail.com",
];

const isWhitelisted = (email) => PRO_WHITELIST.includes(email?.toLowerCase());
...
function AdminDashboard({ businesses, setPage }) {
  ...
  const [whitelistEmails, setWhitelistEmails] = useState([...PRO_WHITELIST]);
  const [newEmail, setNewEmail] = useState("");

  const addToWhitelist = async () => {
    const email = newEmail.trim().toLowerCase();
    if (!email || whitelistEmails.includes(email)) return;
    setWhitelistEmails(prev => [...prev, email]);
    PRO_WHITELIST.push(email);
    if (dbConnected) await SheetDB.addToWhitelist(email);
    setNewEmail("");
  };

  const removeFromWhitelist = async (email) => {
    setWhitelistEmails(prev => prev.filter(e => e !== email));
    const idx = PRO_WHITELIST.indexOf(email);
    if (idx > -1) PRO_WHITELIST.splice(idx, 1);
    if (dbConnected) await SheetDB.removeFromWhitelist(email);
  };
  ...
  const NAV = [
    ...
    { id: "whitelist", icon: "⭐", label: `VIP Access (${whitelistEmails.length})` },
  ];
  ...
  return (
    <div className="dashboard">
      ...
      <div className="main-content">
        ...
        {activeTab === "whitelist" && (
          <div className="fade-in">
            <div className="page-title">⭐ VIP Free Access</div>
            <div className="page-sub">People on this list get full Pro features without paying — only you can see and edit this.</div>

            {/* Add new email */}
            <div className="panel">
              <div className="panel-title">➕ Grant Pro Access</div>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <input
                ...
                <button onClick={addToWhitelist}
                  style={{ padding: "12px 24px", background: "var(--gold)", color: "#0a0a0f", border: "none", borderRadius: 12, fontWeight: 700, fontSize: 14, cursor: "pointer", fontFamily: "DM Sans,sans-serif", flexShrink: 0 }}>
                  Grant Access
                </button>
              </div>
              <div style={{ fontSize: 12, color: "var(--text2)", marginTop: 8 }}>
                They just need to register with this exact email on ReviewShield — Pro features unlock automatically. No payment needed.
              </div>
            </div>

            {/* Current whitelist */}
            <div className="panel">
              <div className="panel-title">✅ Currently Whitelisted ({whitelistEmails.length})</div>
              {whitelistEmails.length === 0 ? (
                <div style={{ fontSize: 14, color: "var(--text2)", padding: "12px 0" }}>No VIP users yet. Add an email above.</div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {whitelistEmails.map((email, i) => {
                    const matchedBiz = businesses.find(b => b.email?.toLowerCase() === email);
                    return (
                      <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 16px", background: "var(--bg3)", borderRadius: 12, border: "1px solid rgba(168,85,247,0.2)", flexWrap: "wrap", gap: 8 }}>
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <span style={{ fontSize: 14, fontWeight: 600 }}>{email}</span>
                            {email === "tarun@thedesignshore.com" && (
                              <span style={{ fontSize: 11, background: "rgba(245,200,66,0.1)", color: "var(--gold)", padding: "2px 8px", borderRadius: 100, fontWeight: 700 }}>YOU</span>
                            )}
                            <span style={{ fontSize: 11, background: "rgba(168,85,247,0.15)", color: "#a78bfa", padding: "2px 8px", borderRadius: 100, fontWeight: 700 }}>⭐ VIP PRO</span>
                          </div>
                          {matchedBiz ? (
                            <div style={{ fontSize: 12, color: "var(--green)", marginTop: 3 }}>✓ Registered as "{matchedBiz.name}" · Active</div>
                          ) : (
                            <div style={{ fontSize: 12, color: "var(--text2)", marginTop: 3 }}>Not registered yet — share the signup link</div>
                          )}
                        </div>
                        {email !== "tarun@thedesignshore.com" && (
                          <button onClick={() => removeFromWhitelist(email)}
                            style={{ fontSize: 12, padding: "6px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.25)", color: "var(--red)", borderRadius: 8, cursor: "pointer", fontFamily: "DM Sans,sans-serif" }}>
                            Revoke Access
                          </button>
                        )}
                      </div>
            ...
            {/* How it works */}
            <div className="panel">
              <div className="panel-title">ℹ️ How This Works</div>
              {[
                { icon: "1️⃣", text: "You add their email here in the admin panel" },
                { icon: "2️⃣", text: "They go to reviewshield.in and register with that exact email" },
                { icon: "3️⃣", text: "They skip payment — Pro features unlock automatically" },
                { icon: "4️⃣", text: "Their dashboard shows a ⭐ VIP badge" },
                { icon: "5️⃣", text: "You can revoke access anytime from here" },
              ].map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: i < 4 ? "1px solid var(--border)" : "none", alignItems: "flex-start" }}>
                  ...
                  <span style={{ fontSize: 14, color: "var(--text2)", paddingTop: 2 }}>{s.text}</span>
                </div>
              ))}
            </div>
          </div>
...
export default function App() {
  ...
  const addFeedback = async (bizId, feedback) => {
    const biz = businesses.find(b => b.id === bizId);
    const fb = {
      id: Date.now(), bizId, bizName: biz?.name || "",
      ...feedback, date: new Date().toISOString().split("T")[0], read: false,
    };
    if (dbConnected) SheetDB.saveFeedback(fb);
    setBusinesses(prev => prev.map(b => b.id === bizId
      ? { ...b, feedbacks: [...b.feedbacks, fb] } : b));
    if (currentBusiness?.id === bizId)
      setCurrentBusiness(prev => ({ ...prev, feedbacks: [...(prev.feedbacks || []), fb] }));
  };

  // ── Increment positivesSent when customer goes to Google ──────────────────
  const addPositive = (bizId) => {
    if (dbConnected) SheetDB.incrementPositive(bizId);
    setBusinesses(prev => prev.map(b => b.id === bizId
      ? { ...b, positivesSent: (b.positivesSent || 0) + 1 } : b));
    if (currentBusiness?.id === bizId)
      setCurrentBusiness(prev => ({ ...prev, positivesSent: (prev.positivesSent || 0) + 1 }));
  };

  const isFullscreen = ["review-preview", "demo-review", "terms", "privacy"].includes(page);

  // ── Loading screen while Sheets data loads ────────────────────────────────
  if (appLoading) return (
    <>
      ...
      <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:"var(--bg)", gap:16 }}>
        <div style={{ fontFamily:"Syne,sans-serif", fontSize:28, fontWeight:800, color:"var(--gold)" }}>ReviewShield</div>
        <div style={{ width:40, height:40, border:"3px solid var(--gold)", borderTopColor:"transparent", borderRadius:"50%", animation:"spin 0.8s linear infinite" }} />
        <div style={{ fontSize:14, color:"var(--text2)" }}>Loading your data...</div>
        <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
      </div>
    </>
  );
  ...
  return (
    <>
      ...
      <div className="app">
        {/* DB status dot */}
        {dbConnected && (
          <div style={{ position:"fixed", bottom:12, left:12, zIndex:999, display:"flex", alignItems:"center", gap:6, background:"rgba(34,197,94,0.1)", border:"1px solid rgba(34,197,94,0.3)", borderRadius:100, padding:"4px 12px", fontSize:11, color:"#22c55e", fontFamily:"DM Sans,sans-serif" }}>
        ...
        {page === "register"      && <RegisterPage setPage={setPage} setCurrentBusiness={setCurrentBusiness} setBusinesses={setBusinesses} dbConnected={dbConnected} />}
        {page === "login"         && <LoginPage setPage={setPage} setCurrentBusiness={setCurrentBusiness} businesses={businesses} />}
        {page === "terms"         && <TermsPage setPage={setPage} />}
        {page === "privacy"       && <PrivacyPage setPage={setPage} />}
        {page === "dashboard"     && currentBusiness && (
          <Dashboard currentBusiness={currentBusiness} setPage={setPage} setCurrentBusiness={setCurrentBusiness} dbConnected={dbConnected} />
        )}
        {page === "admin-login"   && <AdminLogin setPage={setPage} />}
        {page === "admin"         && <AdminDashboard businesses={businesses} setPage={setPage} dbConnected={dbConnected} />}
        {page === "review-preview" && (
          <>
            <div style={{ position:"fixed", top:16, left:16, zIndex:1000 }}>
              <button className="btn-copy" onClick={() => setPage("dashboard")} style={{ fontFamily:"DM Sans,sans-serif" }}>← Back</button>
            </div>
            <ReviewPage businessId={currentBusiness?.id} businesses={businesses} setPage={setPage} addFeedback={addFeedback} addPositive={addPositive} />
          </>
        )}
        {page === "demo-review" && (
          <>
            <div style={{ position:"fixed", top:16, left:16, zIndex:1000 }}>
              <button className="btn-copy" onClick={() => setPage("home")} style={{ fontFamily:"DM Sans,sans-serif" }}>← Back</button>
            </div>
            <ReviewPage isDemo businesses={businesses} setPage={setPage} addFeedback={addFeedback} addPositive={addPositive} />
          </>
        )}

        {/* Hidden admin entry — invisible tap zone bottom-right */}
        {page === "home" && (
